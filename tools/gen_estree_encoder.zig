...
fn writePrologue(w: *Writer) !void {
    try w.print(
        ...
        \\// Pass `srcBuffer` from a prior `decode` to skip re-encoding the
        ...
        \\const HDR_NODE_COUNT_U32 = {[u_nc]d};
...
fn writeRuntime(w: *Writer) !void {
    try w.writeAll(
        \\function encode(estree, srcBuffer) {
        \\  let src = null, dstSrcLen = 0;
        \\  if (srcBuffer) {
        \\    const u8 = new Uint8Array(srcBuffer);
        \\    const u32 = new Uint32Array(srcBuffer, 0, srcBuffer.byteLength >>> 2);
        \\    const sN = u32[HDR_NODE_COUNT_U32];
        \\    const sE = u32[HDR_EXTRA_COUNT_U32];
        \\    const nodesOff = HEADER_SIZE;
        \\    const extraBase = (nodesOff + sN * NODE_SIZE) >>> 2;
        \\    const spOff = nodesOff + sN * NODE_SIZE + sE * 4;
        \\    src = { u8, u32, nodesOff, extraBase, spOff,
        \\            sN, sE, sP: u32[HDR_STRING_POOL_LEN_U32],
        \\            srcLen: u32[HDR_SOURCE_LEN_U32] };
        \\    dstSrcLen = src.srcLen;
        \\  }
        \\  let nodeCap, nodeAB, nU8, nU32, nodeCount;
        \\  let extraCap, extras, extraCount;
        \\  let poolCap, pool, poolLen;
        \\  if (src) {
        \\    nodeCap = Math.max(src.sN + 64, 512);
        \\    nodeAB = new ArrayBuffer(nodeCap * NODE_SIZE);
        \\    nU8 = new Uint8Array(nodeAB);
        \\    nU32 = new Uint32Array(nodeAB);
        \\    nU8.set(src.u8.subarray(src.nodesOff, src.nodesOff + src.sN * NODE_SIZE));
        \\    nodeCount = src.sN;
        \\    extraCap = Math.max(src.sE + 64, 512);
        \\    extras = new Uint32Array(extraCap);
        \\    extras.set(src.u32.subarray(src.extraBase, src.extraBase + src.sE));
        \\    extraCount = src.sE;
        \\    poolCap = Math.max(src.sP + 256, 4096);
        \\    pool = new Uint8Array(poolCap);
        \\    pool.set(src.u8.subarray(src.spOff, src.spOff + src.sP));
        \\    poolLen = src.sP;
        \\  } else {
        \\    nodeCap = 512;
        \\    nodeAB = new ArrayBuffer(nodeCap * NODE_SIZE);
        ...
        \\    const r = { start: dstSrcLen + poolLen, end: dstSrcLen + poolLen + bytes.length };
...
// -------- generic encoder (data-driven from struct layout) -----------
...
// -------- TS signature/callable families --------
...
fn writeDispatcher(w: *Writer) !void {
    try w.writeAll(
        ...
        \\    if (n == null) return NULL;
        \\    if (src !== null) {
        \\      const bufIdx = n[_BUFIDX];
        \\      if (bufIdx !== undefined) {
        \\        const d = n[_DIRTY];
        \\        if (!d || (!d.self && !d.subtree)) return bufIdx;
        \\      }
        \\    }
        \\    switch (n.type) {
        ...
        \\        const v = n.value;
        \\        if (v === null) return enc_null_literal(n);
...
fn writeAssemble(w: *Writer) !void {
    try w.print(
        \\  // ArrowFunctionExpression is generic but the dispatcher above handles
        \\  // it; we just emit the call here for completeness in switch ordering.
