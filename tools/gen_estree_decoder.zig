...
fn generate(w: *Writer) !void {
    try w.writeAll(
        ...
        \\//
        \\// Decodes a Yuku transfer-format buffer into an ESTree object.
    ...
    try writeNodeFunction(w);
    try writeDecodeBody(w);
...
fn writeLookupTables(w: *Writer) !void {
    ...
    try writeArray(w, "COMMENT_TYPES", &meta.COMMENT_TYPES);
...
// estreeType, estreeField, isHoleyArray, snakeConvert live in `estree_meta`.
