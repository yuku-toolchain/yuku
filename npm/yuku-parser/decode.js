...
function decode(buffer, source) {
  ...
  function node(i) {
    ...
    case 86: return { type: "TSBigIntKeyword", start, end };
    ...
    case 100: return { type: "TSTypeParameterInstantiation", start, end, params: nodeArr(f1, f0) };
    case 101: return { type: "TSTypeReference", start, end, typeName: f1 !== NULL ? node(f1) : null, typeArguments: f2 !== NULL ? node(f2) : null };
    case 102: return { type: "TSUndefinedKeyword", start, end };
    case 103: return { type: "TSUnknownKeyword", start, end };
    case 104: return { type: "TSVoidKeyword", start, end };
    case 105: return { type: "JSXElement", start, end, openingElement: f1 !== NULL ? node(f1) : null, children: nodeArr(f2, f0), closingElement: f3 !== NULL ? node(f3) : null };
    case 106: { const r = { type: "JSXOpeningElement", start, end, name: f1 !== NULL ? node(f1) : null, attributes: nodeArr(f2, f0), selfClosing: !!(flags & 1) }; if (_isTs) { r.typeArguments = f3 !== NULL ? node(f3) : null; } return r; }
    case 107: return { type: "JSXClosingElement", start, end, name: f1 !== NULL ? node(f1) : null };
    case 108: return { type: "JSXFragment", start, end, openingFragment: f1 !== NULL ? node(f1) : null, children: nodeArr(f2, f0), closingFragment: f3 !== NULL ? node(f3) : null };
    case 109: return { type: "JSXOpeningFragment", start, end };
    case 110: return { type: "JSXClosingFragment", start, end };
    case 111: return { type: "JSXIdentifier", start, end, name: str(f1, f2) };
    case 112: return { type: "JSXNamespacedName", start, end, namespace: f1 !== NULL ? node(f1) : null, name: f2 !== NULL ? node(f2) : null };
    case 113: return { type: "JSXMemberExpression", start, end, object: f1 !== NULL ? node(f1) : null, property: f2 !== NULL ? node(f2) : null };
    case 114: return { type: "JSXAttribute", start, end, name: f1 !== NULL ? node(f1) : null, value: f2 !== NULL ? node(f2) : null };
    case 115: return { type: "JSXSpreadAttribute", start, end, argument: f1 !== NULL ? node(f1) : null };
    case 116: return { type: "JSXExpressionContainer", start, end, expression: f1 !== NULL ? node(f1) : null };
    case 117: return { type: "JSXEmptyExpression", start, end };
    case 118: { const t = str(f1, f2); return { type: "JSXText", start, end, value: t, raw: t }; }
    case 119: return { type: "JSXSpreadChild", start, end, expression: f1 !== NULL ? node(f1) : null };
    }
  }
