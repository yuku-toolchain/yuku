...
function decode(buffer, source) {
  ...
  function node(i) {
    ...
    case 132: return { type: "TSExportAssignment", start, end, expression: f1 !== NULL ? node(f1) : null };
    case 133: return { type: "TSNamespaceExportDeclaration", start, end, id: f1 !== NULL ? node(f1) : null };
    case 134: return { type: "JSXElement", start, end, openingElement: f1 !== NULL ? node(f1) : null, children: nodeArr(f2, f0), closingElement: f3 !== NULL ? node(f3) : null };
    case 135: { const r = { type: "JSXOpeningElement", start, end, name: f1 !== NULL ? node(f1) : null, attributes: nodeArr(f2, f0), selfClosing: !!(flags & 1) }; if (_isTs) { r.typeArguments = f3 !== NULL ? node(f3) : null; } return r; }
    case 136: return { type: "JSXClosingElement", start, end, name: f1 !== NULL ? node(f1) : null };
    case 137: return { type: "JSXFragment", start, end, openingFragment: f1 !== NULL ? node(f1) : null, children: nodeArr(f2, f0), closingFragment: f3 !== NULL ? node(f3) : null };
    case 138: return { type: "JSXOpeningFragment", start, end };
    case 139: return { type: "JSXClosingFragment", start, end };
    case 140: return { type: "JSXIdentifier", start, end, name: str(f1, f2) };
    case 141: return { type: "JSXNamespacedName", start, end, namespace: f1 !== NULL ? node(f1) : null, name: f2 !== NULL ? node(f2) : null };
    case 142: return { type: "JSXMemberExpression", start, end, object: f1 !== NULL ? node(f1) : null, property: f2 !== NULL ? node(f2) : null };
    case 143: return { type: "JSXAttribute", start, end, name: f1 !== NULL ? node(f1) : null, value: f2 !== NULL ? node(f2) : null };
    case 144: return { type: "JSXSpreadAttribute", start, end, argument: f1 !== NULL ? node(f1) : null };
    case 145: return { type: "JSXExpressionContainer", start, end, expression: f1 !== NULL ? node(f1) : null };
    case 146: return { type: "JSXEmptyExpression", start, end };
    case 147: { const t = str(f1, f2); return { type: "JSXText", start, end, value: t, raw: t }; }
    case 148: return { type: "JSXSpreadChild", start, end, expression: f1 !== NULL ? node(f1) : null };
    }
  }
