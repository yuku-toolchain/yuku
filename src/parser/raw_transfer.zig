...
//
// the binary layout is derived from the AST type definitions at compile time.
// for each node struct, fields are packed in declaration order:
//
//   - bool fields       → 1 bit in the flags byte
//   - enum fields       → ceil(log2(N)) bits in the flags byte
//   - NodeIndex fields  → 1 u32 slot (field1..field5)
//   - IndexRange fields → first: len in u16 (field0), start in u32 slot
//                          subsequent: start then len in consecutive u32 slots
//   - String fields     → 2 u32 slots (start, end)
//   - ?ImportPhase      → 1 flag bit (presence) + enum bits (value)
//   - ?Hashbang         → 1 flag bit (presence) + 2 u32 slots (value.start/end)
//
// the JS decoder is generated from the same type definitions by tools/gen_decoder.zig,
// using the same layout functions, guaranteeing binary format agreement.
//
// binary format:
...
//   nodes  (N * 32 bytes): tag[1] flags[1] field0[2] field1..5[4 each] span_start[4] span_end[4]
//   extra  (M * 4 bytes)
//   string pool (P bytes)
//   comments (C * 20 bytes)
