...
fn parseExpressionStatementWithExpression(
    ...
    const span = parser.builder.getSpan(expression);
...
fn parseLabeledStatement(parser: *Parser, identifier: ast.NodeIndex) Error!?ast.NodeIndex {
    const id_data = parser.builder.getData(identifier);
    const id_span = parser.builder.getSpan(identifier);

    // IdentifierReference to LabelIdentifier
    const label = try parser.builder.createNode(.{
        .label_identifier = .{
    ...
    }, .{ .start = id_span.start, .end = parser.builder.getSpan(body).end });
}
...
pub fn parseBlockStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    return try parser.builder.createNode(.{ .block_statement = .{ .body = body } }, .{ .start = start, .end = end });
}
...
fn parseSwitchCase(parser: *Parser) Error!?ast.NodeIndex {
    ...
    const end = if (consequent.len > 0)
        parser.builder.getSpan(parser.builder.getExtra(consequent)[consequent.len - 1]).end
    else
        colon_end;
...
fn parseCaseConsequent(parser: *Parser) Error!ast.IndexRange {
        ...
        parser.current_token.tag != .eof)
    {
        if (try parseStatement(parser, .{})) |stmt| {
            const stmt_data = parser.builder.getData(stmt);
    ...
    return parser.createExtraFromScratch(&parser.scratch_b, checkpoint);
}
...
pub fn parseIfStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    var end = parser.builder.getSpan(consequent).end;
    ...
    if (parser.current_token.tag == .@"else") {
        ...
        end = parser.builder.getSpan(alternate).end;
    }
...
fn parseEmptyStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    try parser.advance() orelse return null; // consume ';'
    return try parser.builder.createNode(.{ .empty_statement = .{} }, span);
}
...
fn parseBreakStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    if (!parser.canInsertImplicitSemicolon(parser.current_token) and parser.current_token.tag != .semicolon) {
        ...
        label = label_node;
        end = parser.builder.getSpan(label_node).end;
    }

    end = try parser.eatSemicolon(end) orelse return null;

    return try parser.builder.createNode(.{ .break_statement = .{ .label = label } }, .{ .start = start, .end = end });
}
...
fn parseContinueStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    return try parser.builder.createNode(.{ .continue_statement = .{ .label = label } }, .{ .start = start, .end = end });
}
...
fn parseReturnStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    return try parser.builder.createNode(.{ .return_statement = .{ .argument = argument } }, .{ .start = start, .end = end });
}
...
fn parseThrowStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    const end = try parser.eatSemicolon(parser.builder.getSpan(argument).end) orelse return null;

    return try parser.builder.createNode(.{ .throw_statement = .{ .argument = argument } }, .{ .start = start, .end = end });
}
...
fn parseTryStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    if (parser.current_token.tag == .finally) {
        ...
        end = parser.builder.getSpan(finalizer).end;
    }
...
fn parseDebuggerStatement(parser: *Parser) Error!?ast.NodeIndex {
    ...
    return try parser.builder.createNode(.{ .debugger_statement = .{} }, .{ .start = start, .end = end });
}
