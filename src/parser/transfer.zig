// serializes the parsed AST into a compact binary buffer for transfer to JS.
// the JS-side ESTree decoder is generated by tools/gen_estree_decoder.zig using
// the same comptime layout functions defined here, guaranteeing agreement.
//
// each node struct's fields are packed in declaration order:
