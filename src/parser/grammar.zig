...
pub fn validateNoCoverInitializedSyntax(parser: *Parser, expr: ast.NodeIndex) Error!void {
    const data = parser.tree.getData(expr);

    switch (data) {
        .object_expression => |obj| {
            const properties = parser.tree.getExtra(obj.properties);
            for (properties) |prop| {
        ...
        .array_expression => |arr| {
            const elements = parser.tree.getExtra(arr.elements);
            for (elements) |elem| {
        ...
        .sequence_expression => |seq| {
            for (parser.tree.getExtra(seq.expressions)) |e| {
...
pub fn expressionToPattern(
    ...
    switch (data) {
        .identifier_reference => |id| {
            if(context == .binding) {
                parser.tree.replaceData(expr, .{ .binding_identifier = .{
                    .name = id.name,
                } });
            }
        },
        ...
        .assignment_expression => |assign| {
            ...
            parser.tree.replaceData(expr, .{ .assignment_pattern = .{
                .left = assign.left,
                .right = assign.right,
            } });
        },
        ...
        .array_expression => |arr| {
            try array.toArrayPattern(parser, expr, arr.elements, parser.tree.getSpan(expr), context);
        },

        .object_expression => |obj| {
            try object.toObjectPattern(parser, expr, obj.properties, parser.tree.getSpan(expr), context);
        },
        ...
        .spread_element => |spread| {
            ...
            if (parser.tree.getData(arg) == .assignment_pattern) {
                try parser.report(
                    parser.tree.getSpan(expr),
            ...
            parser.tree.replaceData(expr, .{ .binding_rest_element = .{ .argument = arg } });
        },
