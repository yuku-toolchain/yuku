...
pub fn validateNoCoverInitializedSyntax(parser: *Parser, expr: ast.NodeIndex) Error!void {
    ...
    switch (data) {
        .object_expression => |obj| {
            const properties = parser.builder.getExtra(obj.properties);
            for (properties) |prop| {
        ...
        .array_expression => |arr| {
            const elements = parser.builder.getExtra(arr.elements);
            for (elements) |elem| {
        ...
        .sequence_expression => |seq| {
            for (parser.builder.getExtra(seq.expressions)) |e| {
...
pub fn expressionToPattern(
    ...
    switch (data) {
        .identifier_reference => |id| {
            parser.builder.replaceData(expr, .{ .binding_identifier = .{
                .name = id.name,
            } });
        },
        ...
        .assignment_expression => |assign| {
            ...
            parser.builder.replaceData(expr, .{ .assignment_pattern = .{
                .left = assign.left,
                .right = assign.right,
            } });
        },
        ...
        .array_expression => |arr| {
            try array.toArrayPattern(parser, expr, arr.elements, parser.builder.getSpan(expr), context);
        },

        .object_expression => |obj| {
            try object.toObjectPattern(parser, expr, obj.properties, parser.builder.getSpan(expr), context);
        },
        ...
        .spread_element => |spread| {
            ...
            if (parser.builder.getData(arg) == .assignment_pattern) {
                try parser.report(
                    parser.builder.getSpan(expr),
            ...
            parser.builder.replaceData(expr, .{ .binding_rest_element = .{ .argument = arg } });
        },
        ...
        .parenthesized_expression => |paren| {
            ...
            if (!expressions.isSimpleAssignmentTarget(parser, paren.expression)) {
                try parser.report(
                    parser.builder.getSpan(paren.expression),
            ...
            parser.builder.replaceData(expr, parser.builder.getData(paren.expression));
            parser.builder.replaceSpan(expr, parser.builder.getSpan(paren.expression));
        },
